 ...
 * 量表结果AI分析
 */
export const analyzeScaleResult = async (req: Request, res: Response) => {
  try {
    const { scaleTitle, scaleId, questions, answers } = req.body

    if (!scaleTitle || !questions || !answers) {
      return res.status(400).json({
        success: false,
        error: '缺少必要参数'
      })
    ...
    // 构建分析提示词
    const prompt = buildAnalysisPrompt(scaleTitle, scaleId, questions, answers)

    if (useOpenAIFormat) {
      await callAnalysisAPI(API_KEY!, prompt, res)
    } else {
      await callTencentAnalysisAPI(SECRET_ID!, SECRET_KEY!, prompt, res)
    }

  } catch (error) {
    console.error('量表分析错误:', error)
    res.status(500).json({
      success: false,
      error: '分析失败',
      message: error instanceof Error ? error.message : '未知错误'
 ...
 * 构建量表分析提示词
 */
function buildAnalysisPrompt(scaleTitle: string, scaleId: number, questions: any[], answers: Record<string, number>): string {
  // 构建答案摘要
  const answerSummary = questions.map((q, index) => {
    const answer = answers[q.id]
    const selectedOption = q.options.find((o: any) => o.value === answer)
    return `${index + 1}. ${q.title}\n   选项: ${q.options.map((opt: any) => opt.label).join(' | ')}\n   用户选择: ${selectedOption ? selectedOption.label : '未作答'}`
  }).join('\n\n')

  return `你是专业心理咨询师。请根据以下量表测试结果给出简洁分析。
 ...
 * 使用OpenAI格式调用分析API
 */
async function callAnalysisAPI(apiKey: string, prompt: string, res: Response) {
  const systemPrompt = `你是专业心理咨询师。回答要简洁温暖，300字以内。`
  ...
  const data = await response.json()
  const reply = data.choices?.[0]?.message?.content || '抱歉，分析暂时无法完成。'
 ...
 * 使用腾讯云格式调用分析API
 */
async function callTencentAnalysisAPI(secretId: string, secretKey: string, prompt: string, res: Response) {
  const systemPrompt = `你是专业心理咨询师。回答要简洁温暖，300字以内。`
  ...
  const { authorization, headers } = generateTC3Signature(
    ...
    'hunyuan',
    'ChatCompletions',
    JSON.stringify({ Model: 'hunyuan-lite', Messages: messages }),
    timestamp
  )
  ...
  const data = await response.json()
  const reply = data.Response?.Choices?.[0]?.Message?.Content || '抱歉，分析暂时无法完成。'
